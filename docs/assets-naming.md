...
> **This file lives in `docs/`, not in `public/`.** It used to sit at
> `public/assets/NOTES.md`, where Vite copied it verbatim into `dist/` and
> published it — it was live at `/assets/NOTES.md` until 2026-08-07, and one of
> its examples named the real client. Developer notes do not belong in the
> served directory. Anything under `public/` is a public URL.

Static assets live in `public/assets/` and are served from the root URL by Vite.
...
3. In the relevant component, update the `bgImage` prop or `src` attribute:
   ...
   // or
   <img src="/assets/images/services/comparison-before.jpg" alt="Before WPG Parking Lot Solutions" />
   ```
