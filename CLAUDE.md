...
React + Vite static site for WPG Parking Lot Solutions and Concrete, a fictional commercial property maintenance company.
Published to GitHub Pages by `npm run deploy`, which force-pushes `dist/` to the `gh-pages` branch.
...
`public/assets/images/` using the exact filenames defined in `docs/assets-naming.md`.
...
`npm run deploy` force-pushes `dist/` to `gh-pages` as a single orphan commit
(`scripts/deploy.sh`). It deliberately no longer uses the `gh-pages` npm
package: that copies `dist` *over* a cached checkout instead of replacing it,
and its `--remove` default never matches dotfiles, so anything dot-prefixed
published once could never be removed. The branch is now always exactly `dist`.

`dist/.nojekyll` is load-bearing — this repo publishes through *legacy*
branch-based Pages, which runs Jekyll over the branch and hard-fails without
it. It comes from `public/.nojekyll`; the deploy script refuses to run if it
is missing.
...
│
├── public/
│   └── assets/                 ← Static assets served at root. Naming: docs/assets-naming.md
│                                 NOTHING but real site assets belongs here — it is all public URLs.
│       ├── images/
...
│       └── logos/              ← Logo variants: logo-dark.png, logo-light.png,
│                                 favicon.svg + PNG fallbacks (see index.html)
│
├── src/
...
2. Follow naming conventions in `docs/assets-naming.md`
...
- **Design tokens:** `src/styles/variables.css` — cool concrete / white / line-stripe yellow accent (`#F2C21A`).
  The accent is two-tier: `--color-accent` for fills and text on dark, `--color-accent-ink` (`#8A6A00`)
  for accent text on light backgrounds, `--color-on-accent` for text sitting on a fill. Yellow cannot
  carry text on white — do not use `--color-accent` as a text colour on a light section.
...
- **Deployment:** `npm run deploy` force-pushes `/dist` to the `gh-pages` branch as a single orphan commit. Pushing `main` does not publish.
